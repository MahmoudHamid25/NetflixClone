...
export function AllEpisodes({ episodes } : {episodes : ContentResponse}) {
  if (episodes.error) return <div>Error...</div>
  ...
  return (
    <div className="flex flex-col w-full gap-8 mt-8">
      {episodes.data.map((episode, i) => {
        return <MovieCard key={i} title={episode.title!} description={episode.description!} videoUrl={episode.videoUrl!} />;
      })}
    </div>
  );
}
...
export function MovieCard({ title, description, videoUrl }: MovieCardProps) {
  return (
    <Card className="w-full overflow-hidden transition-all duration-300 hover:shadow-xl bg-background-80 dark:text-white">
